...
Logistic regression compared to gam

```{r}
library(mgcv)  # For GAM
library(pROC)  # For ROC-AUC
temp_data <- read.csv("/Users/lizamaguire/Desktop/Thesis_data/Current_data_files_10_21/temp_no_zero.csv")
...
# Fit the GAM model
gam_model_temp <- gam(species_presence ~ s(Bottom.Temperature, k = 5), family = binomial(), data = temp_data)

# Summary of the GAM model
summary(gam_model_temp)


# Fit the logistic regression model
logistic_model_temp <- glm(species_presence ~ Bottom.Temperature, family = binomial(), data = temp_data)

# Summary of the logistic regression model
summary(logistic_model_temp)

# Calculate AIC for both models
gam_aic <- AIC(gam_model_temp)
logistic_aic <- AIC(logistic_model_temp)

cat("AIC for GAM model:", gam_aic, "\n")
cat("AIC for Logistic Regression model:", logistic_aic, "\n")

# Calculate predictions for ROC-AUC comparison
gam_predictions <- predict(gam_model_temp, type = "response")
logistic_predictions <- predict(logistic_model_temp, type = "response")

# Compute ROC and AUC for GAM
roc_gam <- roc(temp_data$species_presence, gam_predictions)
cat("GAM ROC-AUC:", auc(roc_gam), "\n")

# Compute ROC and AUC for logistic regression
roc_logistic <- roc(temp_data$species_presence, logistic_predictions)
cat("Logistic Regression ROC-AUC:", auc(roc_logistic), "\n")
...
Return all GAMs for specified species k= 5, larger font
...
get_all_gams <- function(data, species_name, temp_file_path, sal_file_path, depth_file_path, output_dir) {
  ...
  # Fit the GAM model
  temp_gam_model <- gam(species_presence ~ s(Bottom.Temperature, k = 5), family = binomial(), data = temp_data)
  ...
  sal_gam_model <- gam(species_presence ~ s(Bottom.Salinity, k = 5), family = binomial(), data = sal_data)
  ...
  depth_gam_model <- gam(species_presence ~ s(Depth, k = 5), family = binomial(), data = depth_data)
...
output_directory <- "/Users/lizamaguire/Desktop/R_data/10_23_GAM_photos"
all_gams <- get_all_gams(data, "Hake Atlantic Red",
                          "/Users/lizamaguire/Desktop/Thesis_data/Current_data_files_10_21/temp_no_zero.csv",
                          "/Users/lizamaguire/Desktop/Thesis_data/Current_data_files_10_21/salinity_no_zero.csv",
                          "/Users/lizamaguire/Desktop/Thesis_data/Current_data_files_10_21/depth_no_zero.csv",
                          output_directory)
